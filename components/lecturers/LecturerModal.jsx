...
function LecturerModal({
	...
	const banner = (
		<div
			className='absolute inset-0 flex justify-center items-center z-50 overflow-y-auto
		'>
			New review added!
		</div>
	)
	...
	useEffect(() => {
		function handleResize() {
			setIsSmallScreen(window.innerWidth < 768) // set breakpoint as needed
		}
		...
		return () => {
			window.removeEventListener('resize', handleResize) // remove listener when component unmounts
		}
	}, [])

	return (
		<div
			className='absolute overflow-y-auto inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center z-50'
			onClick={handleCloseModal}>
			{isBannerVisible && banner}
			<div
				className='bg-white grid grid-cols-1 md:grid-cols-2 rounded-lg p-6 max-w-3xl w-full h-full overflow-hidden'
				onClick={(e) => e.stopPropagation()}>
				{/* condition to check if screen is small  */}
				{isSmallScreen ? (
						...
						<div>
							...
							<div className='flex items-center mb-4'>
								<div className={circleStyle}>
									...
									<div
										className={circleTextStyle}>{`${lecturer.avgRating.toFixed(
										1
									)}/5.0`}</div>
								</div>
								<div>
								...
								<div className='flex items-center mb-2 mr-4'>
									...
									<div className='w-full relative'>
										<div
											...
											style={{
												width: `${ratingAverages.coolness * 20}%`,
											}}></div>
									</div>
								</div>
								<div className='flex items-center mb-2 mr-4'>
									...
									<div className='w-full relative'>
										<div
											...
											style={{
												width: `${ratingAverages.grading * 20}%`,
											}}></div>
									</div>
								</div>
								<div className='flex items-center mb-2 mr-4'>
									...
									<div className='w-full relative'>
										<div
											...
											style={{
												width: `${ratingAverages.workload * 20}%`,
											}}></div>
									</div>
								</div>
								<div className='flex items-center mb-2 mr-4'>
									...
									<div className='w-full relative'>
										<div
											...
											style={{
												width: `${ratingAverages.expertise * 20}%`,
											}}></div>
									</div>
								</div>
								<div className='flex items-center mb-2 mr-4'>
